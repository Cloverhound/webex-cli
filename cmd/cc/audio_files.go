...
var audioFilesCmd = &cobra.Command{
	Use:   "audio-files",
	Short: "AudioFiles commands",
}

func init() {
	cmd.CcCmd.AddCommand(audioFilesCmd)
	...
	{ // create
		...
		cmd := &cobra.Command{
			Use:   "create",
			Short: "Create a new Audio File",
			Long:  `Create a new Audio File in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "POST", "/organization/{orgid}/audio-file")
	...
	{ // get-id
		...
		cmd.Flags().StringVar(&orgid, "orgid", "", "Organization ID to be used for this operation. The specified security token must have permission to interact with the organization.")
		cmd.MarkFlagRequired("orgid")
		cmd.Flags().StringVar(&id, "id", "", "Resource ID of the Audio File.")
		cmd.MarkFlagRequired("id")
		cmd.Flags().StringVar(&includeUrl, "include-url", "", "Indicates if the URL for downloading Audio Fileshould be included in the response.")
		audioFilesCmd.AddCommand(cmd)
	}
	...
	{ // list-references
		...
		cmd := &cobra.Command{
			Use:   "list-references",
			Short: "List references for a specific Audio File",
			Long:  `Retrieve a list of all entities that have reference to an existing Audio File by ID in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "GET", "/organization/{orgid}/audio-file/{id}/incoming-references")
				req.PathParam("orgid", orgid)
				req.PathParam("id", id)
	...
	{ // list
		...
		cmd := &cobra.Command{
			Use:   "list",
			Short: "List Audio File(s)",
			Long:  `Retrieve a list of Audio File(s) in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "GET", "/organization/{orgid}/v2/audio-file")
				req.PathParam("orgid", orgid)
		...
		cmd.Flags().StringVar(&attributes, "attributes", "", "Specify the attributes to be returned.Default all attributes are returned along with specified columns. All Attributes are supported except URL")
