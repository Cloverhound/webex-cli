...
func init() {
	...
	{ // list
		...
		cmd := &cobra.Command{
			...
			RunE: func(cmd *cobra.Command, args []string) error {
				...
				req.QueryParam("supervisorView", supervisorView)
	...
	{ // create
		...
		cmd := &cobra.Command{
			...
			Long:  `Create a new Team in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
	...
	{ // bulk-export
		...
		cmd := &cobra.Command{
			Use:   "bulk-export",
			Short: "Bulk export Team(s)",
			Long:  `Export all Team(s) in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "GET", "/organization/{orgid}/team/bulk-export")
				req.PathParam("orgid", orgid)
				req.QueryParam("page", page)
				req.QueryParam("pageSize", pageSize)
				if config.Paginate() {
	...
	{ // list-references
		...
		cmd := &cobra.Command{
			Use:   "list-references",
			Short: "List references for a specific Team",
			Long:  `Retrieve a list of all entities that have reference to an existing Team by ID in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "GET", "/organization/{orgid}/team/{id}/incoming-references")
				req.PathParam("orgid", orgid)
				req.PathParam("id", id)
