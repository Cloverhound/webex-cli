...
var skillProfileCmd = &cobra.Command{
	Use:   "skill-profile",
	Short: "SkillProfile commands",
}

func init() {
	cmd.CcCmd.AddCommand(skillProfileCmd)
 ...
 Note: Array fields are removed from List API. If all fields are required please fetch Id's and use get-by-id API.`,
	...
	{ // create
		...
		cmd := &cobra.Command{
			Use:   "create",
			Short: "Create a new Skill Profile",
			Long:  `Create a new Skill Profile in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
	...
	{ // bulk-save
		...
		cmd := &cobra.Command{
			Use:   "bulk-save",
			Short: "Bulk save Skill Profile(s)",
			Long:  `Create, Update or delete Skill Profile(s) in bulk in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "POST", "/organization/{orgid}/skill-profile/bulk")
				req.PathParam("orgid", orgid)
				if bodyFile != "" {
					if err := req.SetBodyFile(bodyFile); err != nil {
						return err
	...
	{ // bulk-export
		...
		cmd := &cobra.Command{
			Use:   "bulk-export",
			Short: "Bulk export Skill Profile(s)",
			Long:  `Export all Skill Profile(s) in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "GET", "/organization/{orgid}/skill-profile/bulk-export")
				req.PathParam("orgid", orgid)
				req.QueryParam("page", page)
				req.QueryParam("pageSize", pageSize)
				if config.Paginate() {
	...
	{ // get-id
		...
		cmd.Flags().StringVar(&includeSkillDetails, "include-skill-details", "", "If set to true returns skill details")
		skillProfileCmd.AddCommand(cmd)
	}
	...
	{ // list-references
		...
		cmd := &cobra.Command{
			Use:   "list-references",
			Short: "List references for a specific Skill Profile",
			Long:  `Retrieve a list of all entities that have reference to an existing Skill Profile by ID in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "GET", "/organization/{orgid}/skill-profile/{id}/incoming-references")
				req.PathParam("orgid", orgid)
				req.PathParam("id", id)
