...
var multimediaProfileCmd = &cobra.Command{
	Use:   "multimedia-profile",
	Short: "MultimediaProfile commands",
}

func init() {
	cmd.CcCmd.AddCommand(multimediaProfileCmd)
	...
	{ // list
		...
		cmd := &cobra.Command{
			Use:   "list",
			Short: "List Multimedia Profile(s)",
			Long:  `Retrieve a list of Multimedia Profile(s) in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "GET", "/organization/{orgid}/v2/multimedia-profile")
	...
	{ // create
		...
		cmd := &cobra.Command{
			Use:   "create",
			Short: "Create a new Multimedia Profile",
			Long:  `Create a new Multimedia Profile in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "POST", "/organization/{orgid}/multimedia-profile")
				req.PathParam("orgid", orgid)
				if bodyFile != "" {
	...
	{ // bulk-save
		...
		cmd := &cobra.Command{
			Use:   "bulk-save",
			Short: "Bulk save Multimedia Profile(s)",
			Long:  `Create, Update or delete Multimedia Profile(s) in bulk in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "POST", "/organization/{orgid}/multimedia-profile/bulk")
				req.PathParam("orgid", orgid)
				if bodyFile != "" {
					if err := req.SetBodyFile(bodyFile); err != nil {
						return err
	...
	{ // bulk-export
		...
		cmd := &cobra.Command{
			Use:   "bulk-export",
			Short: "Bulk export Multimedia Profile(s)",
			Long:  `Export all Multimedia Profile(s) in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "GET", "/organization/{orgid}/multimedia-profile/bulk-export")
				req.PathParam("orgid", orgid)
				req.QueryParam("page", page)
				req.QueryParam("pageSize", pageSize)
				if config.Paginate() {
	...
	{ // purge-inactive
		...
		cmd := &cobra.Command{
			Use:   "purge-inactive",
			Short: "Purge inactive Multimedia Profile(s)",
			Long:  `Purge inactive Multimedia Profile(s) older than the configured interval for a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "POST", "/organization/{orgid}/multimedia-profile/purge-inactive-entities")
				req.PathParam("orgid", orgid)
	...
	{ // get-id
		...
		cmd.Flags().StringVar(&orgid, "orgid", "", "Organization ID to be used for this operation. The specified security token must have permission to interact with the organization.")
		cmd.MarkFlagRequired("orgid")
		cmd.Flags().StringVar(&id, "id", "", "Resource ID of the Multimedia Profile.")
	...
	{ // list-references
		...
		cmd := &cobra.Command{
			Use:   "list-references",
			Short: "List references for a specific Multimedia Profile",
			Long:  `Retrieve a list of all entities that have reference to an existing Multimedia Profile by ID in a given organization.`,
			RunE: func(cmd *cobra.Command, args []string) error {
				req := client.NewRequest(config.CcBaseURL, "GET", "/organization/{orgid}/multimedia-profile/{id}/incoming-references")
				req.PathParam("orgid", orgid)
				req.PathParam("id", id)
